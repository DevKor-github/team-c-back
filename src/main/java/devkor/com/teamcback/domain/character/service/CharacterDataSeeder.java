...
public class CharacterDataSeeder implements ApplicationRunner {
    ...
    private static final Set<String> LEGACY_POLICY_DESCRIPTIONS = Set.of(
        "기본 아바타",
        "레벨 2 달성 시 해금",
        "레벨 3 달성 시 해금",
        "레벨 4 달성 시 해금",
    ...
    public void run(ApplicationArguments args) {
        ...
        for (KoCharacter seed : seeds) {
            if(characterRepository.existsByName(seed.getName())) {
                KoCharacter existing = characterRepository.findByName(seed.getName()).orElse(null);
                if(existing != null && existing.getDescription() != null
                    && LEGACY_POLICY_DESCRIPTIONS.contains(existing.getDescription())) {
                    existing.updateDescription(seed.getDescription());
        ...
        if(updated > 0) {
            log.info("캐릭터 획득 정책 설명 {}건 갱신 완료", updated);
        }
    }
